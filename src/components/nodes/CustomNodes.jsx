...
import { 
  ...
  ChevronDown,
  X,
  Wrench // Sostituito 'Tool' con 'Wrench' che è disponibile in lucide-react
} from 'lucide-react';

// Base Node UI Component
const BaseNode = ({ icon, label, color, children, onSelect, onDelete, isSelected }) => {
  return (
    <div 
      className={`px-4 py-2 rounded-md shadow-md border ${isSelected ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200'}`}
    ...
    >
      <div className="flex items-center justify-between pb-1 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center">
          {icon}
          <span className="ml-2 font-medium text-gray-800 dark:text-white">{label}</span>
        </div>
        <button 
    ...
    <>
      ...
      >
        <div className="mt-2 grid grid-cols-3 gap-1 text-center text-xs">
          <div className="px-1 py-0.5 bg-blue-100 rounded text-blue-700">Model</div>
          <div className="px-1 py-0.5 bg-teal-100 rounded text-teal-700">Memory</div>
          <div className="px-1 py-0.5 bg-yellow-100 rounded text-yellow-700">Tools</div>
        </div>
      </BaseNode>
    </>
  );
...
export const ConditionNode = ({ data }) => {
  return (
    <>
      ...
      >
        <div className="mt-2 text-xs text-gray-500">
          <div className="mb-1">{data.condition || 'Define a condition...'}</div>
